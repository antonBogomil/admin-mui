const en = {
	translation: {
		HELLO: "Hello, {{name}}!",
		BTN_BACK: "Back",
		BTN_BACK_TO: "Back to Dashboard",
		BTN_PREV: "Prev",
		BTN_NEXT: "Next",
		SAVE: "Save",
		MESSAGE_404: "Sorry, page not found",
		MESSAGE_LOADING_ERROR: "Data loading error",
		MENU_HOME: "Home",
		MENU_ITEMS_MANAGE: "Items manager",
		MENU_USERS: "Users",
		MENU_USERS_LIST: "Users list",
		MENU_USERS_EDIT: "Edit user",
		MENU_USERS_ADD: "Add user",
		MENU_PAGES: "Pages manager",
		MENU_PAGES_LIST: "Pages list",
		MENU_PAGES_ADD: "Create page",
		MENU_SETTINGS: "Site settings",
		MENU_UI_DEMO: "UI Elements",
		MENU_DOC: "Documentation",
		MENU_FORMS: "Forms",
		PAGE_TITLE_HOME: "Home page",
		PAGE_TITLE_USERS: "Users",
		PAGE_TITLE_SETTINGS: "Site settings",
		MENU_UI_DEMO_BTN: "Buttons",
		PAGE_TITLE_MENU: "Menu",
		PAGE_TITLE_PAGES: "List of pages",
		PAGE_TITLE_FORM: "Forms",
		PAGE_TITLE_DOC: "Read documentation",
		INFO_SELECTED: "Selected : ",
		LOGOUT: "Log out",
		SIGN_IN: "Sign in",
		LANGUAGE: "Language",
		LANG_RU: "Russian",
		LANG_EN: "English",
		LANG_UA: "Ukrainian",
		LOGIN: "Login",
		PASSWORD: "Password",
		CANCEL: "Cancel",
		WRONG_PASSWORD: "Wrong password!",
        PASSWORD_NOT_MATCH: "Password does not match!",
        REQUIRED: "This field is required!",
        USE_LESS_CHARS: "Use not more than {{n}} characters!",
        USE_MORE_CHARS: "Use at least {{n}} characters!",
	}
};
const ru = {
	translation: {
		BTN_BACK: "Назад",
		BTN_BACK_TO: "Назад к панели управления",
		BTN_PREV: "Предыдущее",
		BTN_NEXT: "Дальше",
		MESSAGE_404: "Извините, страница не найдена",
		MESSAGE_LOADING_ERROR: "Возникла проблема при загрузке",
		SAVE: "Сохранить",
		MENU_HOME: "Главная",
		MENU_ITEMS_MANAGE: "Управление елементами",
		MENU_USERS: "Пользователи",
		MENU_USERS_LIST: "Список",
		MENU_USERS_EDIT: "Редактировать пользователя",
		MENU_USERS_ADD: "Добавить",
		MENU_PAGES: "Страницы",
		MENU_SETTINGS: "Настройки сайта",
		MENU_UI_DEMO: "UI Елементы",
		MENU_DOC: "Документация",
		MENU_FORMS: "Формы",
		PAGE_TITLE_HOME: "Главная страница",
		PAGE_TITLE_USERS: "Пользователи",
		PAGE_TITLE_SETTINGS: "Настройки сайта",
		MENU_UI_DEMO_BTN: "Кнопки",
		PAGE_TITLE_MENU: "Меню",
		PAGE_TITLE_PAGES: "Список страниц",
		PAGE_TITLE_FORM: "Формы",
		PAGE_TITLE_DOC: "Прочитать документацию",
		INFO_SELECTED: "Выбрано : ",
		LOGOUT: "Выйти",
		SIGN_IN: "Войти",
		LANGUAGE: "Язык",
		LANG_RU: "Русский",
		LANG_EN: "Английский",
		LANG_UA: "Украинский",
		LOGIN: "Логин",
		PASSWORD: "Пароль",
		CANCEL: "Отмнена",
		WRONG_PASSWORD: "Ошибка \"Неверный пароль\""
	}
};
const ua = {
	translation: {
		BTN_BACK: "Back",
		BTN_BACK_TO: "Back to Dashboard",
		BTN_PREV: "Prev",
		BTN_NEXT: "Next",
		MESSAGE_404: "Sorry, page not found",
		MESSAGE_LOADING_ERROR: "Data loading error",
		MENU_HOME: "Home",
		MENU_ITEMS_MANAGE: "Items manager",
		MENU_USERS: "Users",
		MENU_USERS_LIST: "Users list",
		MENU_USERS_EDIT: "Edit user",
		MENU_USERS_ADD: "Add user",
		MENU_PAGES: "Pages",
		MENU_SETTINGS: "Site settings",
		MENU_UI_DEMO: "UI Elements",
		MENU_DOC: "Documentation",
		MENU_FORMS: "Forms",
		PAGE_TITLE_HOME: "Home page",
		PAGE_TITLE_USERS: "Users",
		PAGE_TITLE_SETTINGS: "Site settings",
		MENU_UI_DEMO_BTN: "Buttons",
		PAGE_TITLE_MENU: "Menu",
		PAGE_TITLE_PAGES: "List of pages",
		PAGE_TITLE_FORM: "Forms",
		PAGE_TITLE_DOC: "Read documentation",
		INFO_SELECTED: "Selected : ",

	}
};

export default {
	en, ru, ua
}